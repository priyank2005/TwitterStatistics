...
var twitCount = function(searchTerm, funcPtr) {
    ...
    for (i = 0; i < citiesConfig.length; i++) {
        if (citiesConfig[i].city.toLowerCase() == searchTerm.toLowerCase()) {
            ...
            lat = (parseFloat(coordinates[0]) + parseFloat(coordinates[2])) / 2;
            lng = (parseFloat(coordinates[1]) + parseFloat(coordinates[3])) / 2;
    ...
    //Again Setting to NULL till https://dev.twitter.com/discussions/34372 is closed
    lat = null;
    lng = null;
